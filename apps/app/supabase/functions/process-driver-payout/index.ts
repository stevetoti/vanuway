...
serve(async (req) => {
  ...
  try {
    ...
    if (roleError) throw new Error(`Failed to verify admin role: ${roleError.message}`);
    if (!adminRole) throw new Error("Admin privileges required");
    logStep("Admin caller verified", { userId: caller.id });
