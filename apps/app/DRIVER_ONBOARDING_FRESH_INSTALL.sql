-- =====================================================
-- VANUWAY DRIVER ONBOARDING - FRESH INSTALL
-- =====================================================
-- WARNING: This will DROP existing drivers table!
-- Only run if you don't have production data to preserve
-- =====================================================
...
CREATE INDEX idx_drivers_status ON public.drivers(application_status, is_active);
CREATE INDEX idx_drivers_online ON public.drivers(is_online, is_available);
CREATE INDEX idx_drivers_rating ON public.drivers(average_rating DESC);
...
CREATE INDEX idx_driver_vehicles_type ON public.driver_vehicles(vehicle_type, is_active);
CREATE INDEX idx_driver_vehicles_primary ON public.driver_vehicles(driver_id, is_primary);
...
CREATE INDEX idx_driver_documents_expiry ON public.driver_documents(expires_at) WHERE expires_at IS NOT NULL;
...
CREATE INDEX idx_driver_applications_driver ON public.driver_applications(driver_id);
CREATE INDEX idx_driver_applications_status ON public.driver_applications(status);
CREATE INDEX idx_driver_applications_submitted ON public.driver_applications(submitted_at);
...
CREATE INDEX idx_driver_availability_day ON public.driver_availability(day_of_week, is_active);
...
CREATE INDEX idx_driver_locations_geography ON public.driver_locations USING GIST(location);
...
CREATE INDEX idx_driver_earnings_status ON public.driver_earnings(payment_status);
CREATE INDEX idx_driver_earnings_date ON public.driver_earnings(earned_at DESC);
CREATE INDEX idx_driver_earnings_booking ON public.driver_earnings(booking_id);
...
CREATE INDEX idx_driver_payouts_period ON public.driver_payouts(period_start, period_end);
...
BEGIN
  ...
  RAISE NOTICE '   8 tables created with comprehensive driver management';
  RAISE NOTICE '   Driver profiles, vehicles, documents, GPS tracking, earnings & payouts';
END $$;
