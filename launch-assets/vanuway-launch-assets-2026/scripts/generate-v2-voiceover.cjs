...
const voiceId = env.ELEVENLABS_FEMALE_VOICE_ID || env.ELEVENLABS_Female_VOICE_ID;
...
if (!voiceId) throw new Error("Missing ELEVENLABS_FEMALE_VOICE_ID in .env.local");
...
const script = [
  ...
  "Book rides, send packages, order food, shop from local stores, and buy or sell in the marketplace.",
  "Find hotels, discover tours and attractions, and plan ferry or flight travel between islands.",
  "Learn Bislama, stay informed with emergency alerts, discover community events, and connect with trusted local service providers.",
  "Browse real estate, access VanuHealth services, find jobs, and hire local talent through VanuJobs.",
  "For businesses, VanuWay helps you manage listings, customer messages, approvals, activity, and promotions from one dashboard.",
  "VanuWay is launching now.",
  "Visit www.vanuway.com and download the app from there."
].join(" ");
...
async function main() {
  const output = path.join(outDir, "vanuway-launch-v2-voiceover-female.mp3");
  await synthesize({ text: script, output });
  fs.writeFileSync(path.join(outDir, "vanuway-launch-v2-script.txt"), `${script}\n`);
